...
class StockDataFrame(pd.DataFrame):
    ...
    def _get_omacd(self):
        """ 0 lag Moving Average Convergence Divergence
        ...
        # FAST LINE
        ema1= self._ema(close, self.MACD_EMA_SHORT)
        ema2 = self._ema(ema1, self.MACD_EMA_SHORT)
        differenceFast = ema1 - ema2
        zerolagEMA = ema1 + differenceFast
        demaFast = (2 * ema1) - ema2
        
         # SLOW LINE
        emas1= self._ema(close, self.MACD_EMA_LONG)
        emas2 = self._ema(emas1, self.MACD_EMA_LONG)
        ...
        # SIGNAL LINE
        emasig1 = self._ema(ZeroLagMACD, self.MACD_EMA_SIGNAL)
        emasig2 = self._ema(emasig1, self.MACD_EMA_SIGNAL)
        signal = (2 * emasig1) - emasig2
        
        close = self['close']
        ema_short = self._ema(close, self.MACD_EMA_SHORT)
        ema_long = self._ema(close, self.MACD_EMA_LONG)
        self['omacd'] = ZeroLagMACD
        self['omacds'] = signal
        self['omacdh'] = self['omacd'] - self['omacds']
    ...
    def handler(self):
        return {
            ...
            ('omacd', 'omacds', 'omacdh'): self._get_omacd,
